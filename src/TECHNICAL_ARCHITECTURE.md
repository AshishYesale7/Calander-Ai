# Azure App Registration - Internal Notes

== Application: Calendar.ai ==

**Purpose:** This App Registration handles all Microsoft-based authentication and Microsoft Graph API integration for the Calendar.ai web application. It enables user sign-in via personal and work/school accounts and provides API access for features like calendar and email sync.
...
**Key Configuration:**
-   **Homepage:** http://futuresight-8ebba4468907.herokuapp.com
-   **Redirect URI (Web):** `https://{your-domain}/api/auth/microsoft/callback` (Must match `NEXT_PUBLIC_BASE_URL` in app config).
-   **Tenant:** `/common` (Supports both Personal and Work/School accounts).

**Permissions Rationale:**
A broad set of scopes (User.Read, Calendars.ReadWrite, Mail.Read, Files.ReadWrite.All) are requested to support current and future features like timeline sync, email summaries, and document integration.

**Troubleshooting:**
-   **Unverified Publisher:** If this warning appears, ensure the app's domain is verified in Azure by hosting the `microsoft-identity-association.json` file.
-   **Redirect URI Mismatch:** Ensure the URI in Azure matches the app's `NEXT_PUBLIC_BASE_URL` + `/api/auth/microsoft/callback` exactly.
