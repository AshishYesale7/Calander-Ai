...
import { format, startOfWeek, addDays, isToday as dfnsIsToday } from 'date-fns';
...
interface StreakPopoverContentProps {
    streakData: StreakData;
}

export default function StreakPopoverContent({ streakData }: StreakPopoverContentProps) {
    const { isLoading } = useStreak();
    ...
    const weekDaysWithStatus = useMemo(() => {
        if (!streakData) {
            return weekDays.map(day => ({ dayChar: format(day, 'E').charAt(0), isCompleted: false }));
        }
        const completedDaysSet = new Set(streakData.completedDays || []);
        return weekDays.map((day) => {
            const dayStr = format(day, 'yyyy-MM-dd');
            if (dfnsIsToday(day)) {
                return { dayChar: format(day, 'E').charAt(0), isCompleted: streakData.todayStreakCompleted };
            }
            return { dayChar: format(day, 'E').charAt(0), isCompleted: completedDaysSet.has(dayStr) };
        });
    }, [weekDays, streakData]);
    ...
    return (
        <div className="relative">
            ...
            <div className="p-4 relative z-10">
                <div>
                    <h3 className="text-sm font-bold">Daily Goal</h3>
                    <div className="text-xs text-muted-foreground mt-1 h-4">
                        {isLoading && !streakData.insight?.text ? (
