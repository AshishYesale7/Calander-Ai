...
// SVG icon component for Google, combining multiple icons for a branded look.
const GoogleIcon = () => (
...
// SVG icon component for Microsoft.
const MicrosoftIcon = () => (
  <div className="flex items-center gap-1.5 mr-2">
...
// Defines a global interface on the `Window` object for Firebase reCAPTCHA and OTP results.
declare global {
  interface Window {
...
// This is the main component for the sign-in form.
export default function SignInForm({ avatarUrl }: SignInFormProps) {
  // Next.js router for navigation.
  const router = useRouter();
  // Custom hook for showing toast notifications.
  const { toast } = useToast();
  // State to manage which button is in a loading state.
  const [loading, setLoading] = useState<string | null>(null);
  // Custom hook to get the current authentication state and user.
  ...
  // This effect handles the countdown timer for resending OTP.
  useEffect(() => {
    if (countdown > 0) {
  ...
  // This effect listens for a message from the OAuth popup window to know when to reload the page on success.
  useEffect(() => {
  ...
  // Handles sign-in with social providers (Google, Microsoft, etc.).
  const handleProviderSignIn = async (providerName: 'google' | 'microsoft' | 'yahoo' | 'apple') => {
     // Apple sign-in is shown as disabled for now.
    ...
    setLoading(providerName); // Set loading state for the specific button.

    try {
        let provider;
        // Configure the correct Firebase provider based on the button clicked.
        if (providerName === 'google') {
            provider = new GoogleAuthProvider();
            // Request necessary scopes for Google services.
        ...
        // Trigger the Firebase sign-in popup.
        ...
        if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/cancelled-popup-request') {
          ...
          // This error occurs if the sign-in method is not enabled in the Firebase console.
          ...
          toast({
            ...
            // This case is important for user experience. It guides the user on how to sign in if their email is already associated with a different method.
  ...
  const setupRecaptcha = () => {
    ...
    // Clear any previous verifier to avoid errors.
    if (window.recaptchaVerifier) {
      window.recaptchaVerifier.clear();
    }
    
    // Create and render a new reCAPTCHA verifier instance.
  ...
  // Handles sending the OTP to the user's phone number.
  const handleSendOtp = async (isResend = false) => {
    if (isResend && countdown > 0) return; // Prevent spamming the resend button.
    ...
    setLoading('phone');
    try {
      await setupRecaptcha(); // Set up reCAPTCHA before sending OTP.
      const appVerifier = window.recaptchaVerifier!;
      const confirmationResult = await signInWithPhoneNumber(auth, phone, appVerifier);
      window.confirmationResult = confirmationResult; // Store the result for OTP verification.
      
      if (!isResend) {
        setStep('otp'); // Move to the OTP entry step.
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading('otp');
    try {
      const userCredential = await confirmationResult.confirm(otp); // Confirm the OTP.
      toast({ title: "Sign In Successful!", description: "Welcome back to Calendar.ai." });
  ...
  // If authentication is still loading or user is already logged in, show a spinner.
  if (authLoading || user) {
      return (
  ...
  return (
    <Card className="frosted-glass p-6 md:p-8 w-full max-w-sm ml-0 md:ml-12 lg:ml-24">
      ...
      <CardContent className="p-0">
        ...
        {/* This part conditionally renders either the phone input or the OTP input. */}
        {step === 'phone' ? (
