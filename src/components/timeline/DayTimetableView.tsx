...
interface AllDayEventWithLayout extends TimelineEvent {
  row: number;
  span: number;
  startDay: number;
}
function calculateAllDayEventLayouts(allDayEvents: TimelineEvent[], week: Date[]): { events: AllDayEventWithLayout[], maxRows: number } {
    ...
    const eventsInWeek = allDayEvents
        .map(e => ({...e, start: dfnsStartOfDay(e.date), end: dfnsStartOfDay(e.endDate || e.date)}))
        .filter(e => {
    ...
    return (
      <div className={cn("flex flex-col flex-1 w-full text-xs h-full", themeClasses.container)}>
        <div className={cn("sticky top-0 z-20 flex-shrink-0", themeClasses.headerContainer)}>
            ...
            <div className={cn("grid grid-cols-[3rem_repeat(7,1fr)] text-xs border-b", themeClasses.headerCell)} style={{ minHeight: `${(allDayLayouts.maxRows + 1) * 24}px`}}>
                ...
                <div className="grid grid-cols-7 col-span-7 p-1 gap-1 relative">
                                        ...
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent className="frosted-glass">
                                                ...
                                                <AlertDialogFooter>
                                                    ...
                                                    <AlertDialogAction className="bg-destructive hover:bg-destructive/90" onClick={() => onDeleteEvent(event.id, event.title)}>Delete</AlertDialogAction>
                                                </AlertDialogFooter>
                                            </AlertDialogContent>
                                        </AlertDialog>
                                        )}
...
const DayView = ({
  ...
  const themeClasses = {
      ...
      allDayArea: viewTheme === 'dark' ? 'bg-[#1c1c1c] border-b border-gray-700/50' : 'bg-[#faefdd] border-b border-stone-200',
